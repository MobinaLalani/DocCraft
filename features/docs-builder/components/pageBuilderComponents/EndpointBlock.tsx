...
function createCurlCommand(component: EndpointComponent) {
  const lines = [
    `curl -X ${shellQuote(component.method)} \\`,
    `  ${shellQuote(component.path)} \\`,
    `  -H ${shellQuote(`accept: ${component.responseContentType ?? "application/json"}`)}`,
  ];
  const requestBody = component.requestExample?.trim();

  if (requestBody) {
    lines[lines.length - 1] += " \\";
    lines.push(
      `  -H ${shellQuote(`Content-Type: ${component.requestContentType ?? "application/json"}`)} \\`,
...
function CurlPanel({ component }: { component: EndpointComponent }) {
  const [copied, setCopied] = useState(false);
  const curlCommand = createCurlCommand(component);
  ...
  return (
    <section className="border-t border-slate-200 px-5 py-5 sm:px-7">
      <div className="mb-3 flex items-center justify-between gap-3">
        <h4 className="text-sm font-bold text-slate-800">cURL</h4>
        <button
          type="button"
          onClick={copyCurl}
          className="inline-flex items-center gap-1.5 rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 text-xs font-medium text-slate-600 transition hover:border-slate-400 hover:text-slate-900"
        >
          {copied ? <Check size={14} aria-hidden="true" /> : <Copy size={14} aria-hidden="true" />}
          {copied ? "کپی شد" : "کپی"}
        </button>
      </div>
      <pre className="overflow-x-auto rounded-md bg-[#263238] p-4 font-mono text-xs leading-6 text-[#e6e6e6]">
...
export function EndpointBlock({ component }: PageBlockProps<EndpointComponent>) {
  ...
  return (
    <article dir="ltr" className={`overflow-hidden rounded-xl border ${config.border} bg-white text-left shadow-sm`}>
